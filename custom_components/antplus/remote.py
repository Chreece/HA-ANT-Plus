...
from .const import REMOTE_CAPTURE_STATE_EVENT, REMOTE_GATEWAY_HELLO_EVENT, REMOTE_PACKET_EVENT
...
def _process_remote_packet(receiver: AntPlusReceiver, packet: dict[str, Any], gateway_id: str) -> None:
...
def async_register_remote_listener(hass: HomeAssistant, receiver: AntPlusReceiver) -> Callable[[], None]:
    ...
    def handle_packet_event(event: Event) -> None:
        ...
        if packets is None:
            ...
            _LOGGER.warning("Ignoring remote ANT+ event from %s: packets must be a list", gateway_id)
            return
        for packet in packets:
            ...
            try:
                ...
                _LOGGER.warning("Ignoring invalid ANT+ packet from gateway %s: %s", gateway_id, err)

    @callback
    def handle_gateway_hello(event: Event) -> None:
        gateway_id = str(event.data.get("gateway_id", "")).strip()
        _LOGGER.info("Remote ANT+ gateway connected: %s", gateway_id or "unknown")
        fire_capture_state(gateway_id or None)
