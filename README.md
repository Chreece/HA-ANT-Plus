...
Active controls are created only for profiles with a verified command implementation and only become available while an adapter that has actually received the device is active. Controls Device exposes the documented Generic Control commands: menu Up/Down/Select/Back/Home and timer Start/Stop/Reset/Lap/Length. Fitness Equipment (FE-C) exposes Target Power, Basic Resistance, simulation grade/rolling resistance/wind/drafting, user/bicycle/wheel/gear configuration, capability requests and calibration controls; received FE-C capabilities disable modes the equipment explicitly reports as unsupported. Bicycle Power exposes manual calibration. LEV exposes assist/regenerative level, front/rear gear command, wheel circumference, command manufacturer ID, lights, high beam and turn signals. Dropper Seatpost exposes valve unlock/lock and stored unlock delay. Tire Pressure Monitor exposes barometric/low/high pressure settings and sensor position. Commands are routed through the local ANT USB adapter or the exact remote HA ANT+ gateway/adapter that observed the device. Remote gateways advertise a control protocol and return a correlated success/error result for every command, so HA does not treat fire-and-forget delivery as success. Advanced actions `antplus.send_generic_control`, `antplus.send_raw_control` and `antplus.request_data_page` use this same confirmed local/remote route. Generic timer Stop is not the same command as Audio/Video Pause; ANT+ Audio/Video Controls are a separate Controls use case and are only sent through the raw-control action until their exact profile-specific packet surface is implemented.
